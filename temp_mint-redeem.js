...
const redeemValueInput = document.getElementById("user-redeem-value");
redeemValueInput.disabled = false;
redeemValueInput.value = "Please connect wallet";
...
async function connect() {
  try {
    ...
    if (networkId == 42161) {
        ...
        redeemValueInput.disabled = false;
        redeemValueInput.value = "";
	    
	      // Enable buttons
	    toggleButtons(true);
	    await getWalletTokenBalances();
	    await updateTargetTokenButtons();
	} else {
...
async function getWalletTokenBalances() {
  ...
  try {
    ...
    document.getElementById("ussd-balance").textContent = balanceUSSD.toFixed(decimalsUSSD);
    ...
    document.getElementById("usdt-balance").textContent = balanceUSDT.toFixed(decimalsUSDT);
    ...
    document.getElementById("wbtc-balance").textContent = balanceWBTC.toFixed(decimalsWBTC);
    ...
    document.getElementById("weth-balance").textContent = balanceWETH.toFixed(decimalsWETH);
...
function toggleButtons(enabled) {
  const buttons = ["btnMint", 'btnRedeem', 'btnUSDTSelector', 'btnWBTCSelector', 'btnWETHSelector'];
...
async function updateTargetTokenButtons() {
	...
	if (cycle >= 52500 && cycle <= 105000) {
		...
		const usdtDecimals = await contractUSDT.methods.decimals().call();
		const ussdDecimals = await contractUSSD.methods.decimals().call();

		// Получение баланса USDT на контракте USSD
		const usdtBalanceRaw = await contractUSDT.methods.balanceOf(contractAddressUSSD).call();
		// Преобразование с учетом десятичных знаков
		const usdtBalance = usdtBalanceRaw / (10 ** usdtDecimals);

		// Получение общего объема эмиссии USSD
		const totalSupplyRaw = await contractUSSD.methods.totalSupply().call();
		// Преобразование с учетом десятичных знаков
		const totalSupply = totalSupplyRaw / (10 ** ussdDecimals);
...
async function checkApprovalAndCalculateRequiredAmount() {
    // Шаг 2: Расчет необходимого количества токенов для минтинга (Spending Amount)
    /*let spendingAmount;
    if (targetToken === 'USDT') {
        spendingAmount = await contractUSSD.methods.calculateMint(contractAddressUSDT, amount * Math.pow(10, 6)).call()
    } else if (targetToken === 'WBTC') {
        spendingAmount = await contractUSSD.methods.calculateMint(contractAddressWBTC, amount * Math.pow(10, 8)).call()
    } else if (targetToken === 'WETH') {
        spendingAmount = await contractUSSD.methods.calculateMint(contractAddressWETH, amount * Math.pow(10, 18)).call()
    }*/
}
...
async function redeem() {
    const amountWithDecimals = Math.round(redeemValueInput.value.replace(',', '.') * Math.pow(10, 6));
    contractUSSD.methods.redeem(amountWithDecimals, accounts[0])
        .send({ from: accounts[0] })
        .on('transactionHash', function(hash){
            console.log('Transaction hash:', hash);
        })
        .on('receipt', async function(receipt){
            console.log('Transaction was approved successfully:', receipt);
            const event = receipt.events.Redeem;
            if (event) {
                const stableCoinAmount = event.returnValues.amountUSSD / Math.pow(10, 6);

                console.log('Redeemed USSD:', stableCoinAmount);
                customAlert(`You have redeemed ${stableCoinAmount} USSD`);

                document.getElementById('user-redeem-value').value = '';
                await getWalletTokenBalances();
            }
        })
        .on('error', function(error, receipt) {
